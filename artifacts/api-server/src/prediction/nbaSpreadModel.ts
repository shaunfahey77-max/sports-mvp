 ...
 * Estimates cover probability for each side at the published spread.
 *
 * Uses a normal distribution over expected margin to compute cover probabilities.
 * Margin std dev is calibrated to NBA game distribution (~12 points).
 */
...
export async function predict(game: GameMarketInput): Promise<ModelOutput> {
  ...
  const expectedMargin = probToMargin(fairHome, MARGIN_STD_DEV) + HOME_ADVANTAGE[LEAGUE] * 5;

  // Negate the spread: publishSpread is the home team's handicap point (e.g. -4.5 means
  // home must win by 5+). Home covers if actual_margin > |spread|, i.e. margin > -publishSpread.

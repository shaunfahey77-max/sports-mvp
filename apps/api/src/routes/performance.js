...
async function loadClvRows({ leagues, days }) {
  const start = startDateFromDays(days);
  const end = ymdUTC(new Date());

  const { data, error } = await supabase
    .from("picks_daily")
    .select("date,league,pick,result,clv_line_delta,clv_implied_delta")
    .gte("date", start)
    .lte("date", end)
    .in("league", leagues);
...
function buildClvMap(clvRows) {
  ...
  for (const row of clvRows) {
    ...
    if (imp != null) {
      bucket.impClvCount += 1;
      bucket.impClvSum += imp;
    }
  }
...
function mergePerformanceWithClv(perfRows, clvRows) {
  const clvMap = buildClvMap(clvRows);
...
async function loadMerged({ leagues, days }) {
  const perf = await loadPerformanceRows({ leagues, days });
  const clv = await loadClvRows({ leagues, days });
  const rows = mergePerformanceWithClv(perf.rows, clv);

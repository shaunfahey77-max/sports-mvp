...
// Small per-host concurrency gate
 ...
 * - cache
 * - in-flight de-dupe
 * - retry/backoff on 429 / 5xx / transient errors
...
async function fetchJson(
  url,
  { headers } = {},
  { cacheTtlMs = CACHE_TTL_MS, retries = 5, baseBackoffMs = 650, hostConcurrency = 2, timeoutMs = 25_000 } = {}
) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (res.status === 429) {
          ...
          const waitMs =
            retryAfter != null ? retryAfter * 1000 : jitter(baseBackoffMs * Math.pow(2, attempt), 0.25);

          if (attempt >= retries) {
            throw new Error(`Upstream 429 for ${url}${text ? ` — ${text}` : ""} — exceeded retry limit`);
          }
          await sleep(waitMs);
          continue;
        }
...
/**
 * Vegas helpers (The Odds API) — NBA anchoring only
 */
...
async function fetchNbaVegasForDate(ymd) {
  ...
  let data = await fetchJson(url, {}, { cacheTtlMs: HEAVY_CACHE_TTL_MS, retries: 1, timeoutMs: 7_000, hostConcurrency: 1 })
...
function buildWhyPanel({ pickSide, pickNote, edgeForPick, conf, deltas = [] }) {
  ...
  if (!pickSide) {
    bullets.push(
      pickNote === "pass_toss_up" ? "Pass: edge not strong enough (toss-up)." : "Pass: insufficient signal."
    );
  ...
  bullets.push(`Pick: ${pickSide.toUpperCase()} (edge ${Number.isFinite(edgeForPick) ? edgeForPick.toFixed(3) : "—"})`);
...
function toUnifiedGame({ league, gameId, date, status, home, away, pickSide, pickNote, edgeForPick, conf, tier, whyPanel, factors }) {
...
async function getNbaGamesByDate(dateYYYYMMDD) {
  ...
  return rows.map((g) => {
    ...
    return {
      ...
      status: g?.status || "",
      home: { id: toNbaTeamId(homeAbbr), name: homeAbbr || "", abbr: homeAbbr, score: homeScore },
      away: { id: toNbaTeamId(awayAbbr), name: awayAbbr || "", abbr: awayAbbr, score: awayScore },
      homeScore,
      awayScore,
    };
  });
  ...
  return computeCached(key, HEAVY_CACHE_TTL_MS, async () => {
    ...
    for (const g of schedule) {
      ...
      const rawHomeValueEdge =
        Number.isFinite(pHomeAdj) && Number.isFinite(pHomeMkt) ? pHomeAdj - pHomeMkt : mv === "v2" ? pHomeAdj - 0.5 : statEdge;
      ...
      const whyPanel = buildWhyPanel({ pickSide: pick.side, pickNote: pick.note, edgeForPick: Number.isFinite(edgeForPick) ? edgeForPick : null, conf, deltas });
      ...
      games.push(
        toUnifiedGame({
          ...
          factors: {
            ...
            note:
              mv === "v2"
                ? "NBA v2+: stat hybrid → probability + market anchoring + shrunk/clamped value edge + uncertainty-aware thresholds + winProb-gated tiers."
                : "NBA v1: win% + margin + recent 10 + recent 5 margin + small home adv. PASS discipline.",
          },
        })
      );
    }
...
// NHL placeholder (you can wire later)
async function buildNhlPredictions(dateYYYYMMDD, windowDays) {
  return {
    meta: {
      ...
      model: "NHL paused-v1",
      mode: "regular",
      note: "NHL paused — no games scheduled / model disabled.",
      warnings: [],
      elapsedMs: 0,
    },
    games: [],
...
/* =========================================================
   NCAAM — ESPN (CANONICAL FINAL FIX)
   ========================================================= */
...
function normalizeEspnEventToGame(event) {
  ...
  // ✅ CANONICAL STATUS:
  // If ESPN says completed=true OR status looks final-ish AND scores exist -> status="final"
  const homeScore = espnCompetitorScore(home);
  const awayScore = espnCompetitorScore(away);
  const hasScores = Number.isFinite(homeScore) && Number.isFinite(awayScore) && homeScore + awayScore > 0;

  let status = "scheduled";
  if (completed && hasScores) status = "final";
  else if ((state === "post" || state.includes("final") || state.includes("status_final")) && hasScores) status = "final";
  else if (state === "in") status = "in";
  else if (state === "pre") status = "scheduled";
  else status = state || "scheduled";
...
async function buildNcaamPredictions(dateYYYYMMDD, windowDays, { tournamentMode, modeLabel }) {
  ...
  for (const g of slate) {
    ...
    const edgeForPick = pick.side ? Math.abs(edgeSigned) : null;
    ...
    games.push(
      toUnifiedGame({
        ...
        gameId: `ncaam-${g.id}`,
        date: dateYYYYMMDD,
        // ✅ CANONICAL: already "final" for completed finals
        ...
        factors: {
          ...
          espnRawState: g.raw?.espnState ?? null,
          espnCompleted: g.raw?.espnCompleted ?? null,
          note: tournamentMode
...
router.get("/nba/predict", async (req, res) => {
  ...
  try {
    ...
    res.json(addPredictionsAlias(out));
  } catch (e) {
    res.json(
      addPredictionsAlias({
        meta: {
...
router.get("/predictions", async (req, res) => {
  ...
  try {
    if (league === "nba") {
      ...
      const modelVersion = readModelFromReq(req, "v2");
      const out = await buildNbaPredictions(date, windowDays, { modelVersion });
      return res.json({ ok: true, league, date, count: out.games.length, ...addPredictionsAlias(out) });
    }
    if (league === "nhl") {
      const windowDays = readWindowFromReq(req, 60, 3, 120);
      const out = await buildNhlPredictions(date, windowDays);
...
// Export builders so /api/performance can call them directly
